...
function MenuContent({ name, getHref, onNavigate, isMega }) {
  const menu = menus[name];
  const columns = (!isMega && menu.columnsCompact) ? menu.columnsCompact : menu.columns;
  const cardsList = menu.cards && <div className="vv-cards">{menu.cards.map(card => <a className="vv-card" key={card.title} href={getHref(card.title, name)} onClick={() => onNavigate?.(card.title)}>
  ...
  return <div className={`vv-menu vv-menu--${slug(name)}`}>
    <div className="vv-columns">
      {columns.map(column => <section className="vv-column" key={column.title}>
        <h2>{column.title}</h2>
    ...
    {menu.featured ? <div className="vv-sidebar">
      <a className="vv-featured" href={getHref(menu.featured.title, name)} onClick={() => onNavigate?.(menu.featured.title)}>
        <img src={asset(menu.featured.image)} alt={menu.featured.title}/>
        <div className="vv-featured-copy">
          <span className="vv-featured-eyebrow">{menu.featured.eyebrow}</span>
          <span className="vv-featured-title">{menu.featured.title}</span>
          <span className="vv-featured-cta">{menu.featured.cta}</span>
        </div>
      </a>
      {cardsList}
    </div> : cardsList}
...
export default function VivaVoceHeader({ initialMenu = null, getHref = defaultHref, onNavigate }) {
  ...
  const [isMega, setIsMega] = useState(() => typeof window === 'undefined' || window.matchMedia('(min-width: 1440px)').matches);
  ...
  useEffect(() => {
    ...
    const update = () => setIsMega(media.matches);
  ...
  return <header className="vv-header" ref={root} onBlur={event => {
    ...
    <div className="vv-desktop" onPointerEnter={cancelClose} onPointerLeave={event => { if (event.pointerType === 'mouse' && !event.currentTarget.contains(document.activeElement)) closeTimer.current = setTimeout(() => setActive(null), 180); }}>
      ...
      {Object.keys(menus).map(name => <div key={name} ref={el => { desktopPanelRefs.current[name] = el; }} className={`vv-desktop-panel${active === name ? ' is-open' : ''}`} aria-hidden={active !== name} id={`${id}-${slug(name)}-panel`} aria-labelledby={`${id}-${slug(name)}-tab`}><MenuContent name={name} getHref={getHref} onNavigate={navigate} isMega={isMega}/></div>)}
    </div>
    <nav className={`vv-mobile-nav${mobileOpen ? ' is-open' : ''}`} id={`${id}-mobile`} aria-label="Mobile navigation" aria-hidden={!mobileOpen}>
      {navItems.map(name => menus[name] ? <div key={name} className="vv-mobile-section"><button className="vv-mobile-heading" aria-expanded={mobileSection === name} aria-controls={`${id}-mobile-${slug(name)}`} onClick={() => toggleMobileSection(name)}>{name}<span aria-hidden="true">{mobileSection === name ? '−' : '+'}</span></button><div ref={el => { mobileBodyRefs.current[name] = el; }} className="vv-mobile-section-body" id={`${id}-mobile-${slug(name)}`} aria-hidden={mobileSection !== name}><MenuContent name={name} getHref={getHref} onNavigate={navigate} isMega={isMega}/></div></div> : <a className="vv-mobile-heading" key={name} href={getHref(name)} onClick={() => navigate(name)}>{name}</a>)}
